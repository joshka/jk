...
- optional local split is acceptable only if it materially improves exact local/remote state reading
...
- no broad remote dashboard
- no action targeting from rendered `name@remote` labels alone
- no local or remote mutation path without exact typed bookmark metadata
