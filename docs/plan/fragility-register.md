...
## Current Code
  ...
  | Rebase preview flow       | Explicit source change ids, current-row destination, and `jj rebase` command semantics                                         | Selected-source ordering or CLI behavior can drift from the intended graph move  | Preserve source/destination roles through the prompt, build the exact `jj rebase -r ... -o ...` shape, and reveal a rebased source after refresh                     |
