...
Status: source audit on 2026-09-20 of unlanded work in `coherence`, `review-refs`,
`review-execution`, and `review-selectors`. This describes their combined implementation scope,
not a released version or evidence that the branches have been integrated and validated together.
...
| Bookmarks              | List, create, move, delete in refs work                                 | Track, untrack, forget, advance, conflict resolution             |
| Tags                   | Typed commands                                                          | Native list and scoped actions                                   |
| Git fetch/push         | Explicit remote fetch; one-bookmark push dry-run in refs work           | Real push is disabled; advanced remote forms                     |
...
The refs work restricts bookmark mutation, fetch, and push command-mode entry to its dedicated
...
- Execution work provides captured shell-free external commands and cancellable log refresh.
