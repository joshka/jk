...
fn split_exact_change_uses_exact_revision_option() {
    ...
    assert!(
        split
            .preview_summary()
            .contains("target: exact selected log revision tvykuurwpnwzzqulzrvwvmxxotnlywqw")
    );
}
...
fn edit_plan_uses_exact_change_id_revset() {
    ...
    assert!(
        plan.preview_summary()
            .contains("target: exact selected log revision change-a")
    );
    assert!(
        plan.preview_summary()
...
fn next_plan_uses_explicit_edit_flag_and_ignores_selection() {
    ...
    assert!(
        plan.preview_summary()
            .contains("highlighted log row is not an argument to jj next --edit")
    );
    assert!(
        plan.preview_summary()
...
fn prev_plan_uses_explicit_edit_flag_and_mentions_ambiguity() {
    ...
    assert!(
        plan.preview_summary()
            .contains("highlighted log row is not an argument to jj prev --edit")
    );
    assert!(
        plan.preview_summary()
