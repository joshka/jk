...
fn log_view(entries: Vec<LogItem>) -> LogView {
    LogView::test_new(entries)
}
...
fn copy_options_use_row_semantics() {
    let view = log_view(vec![log_item("row text", Some("change"), Some("commit"))]);
...
fn current_revset_is_none_for_non_selectable_log_rows() {
    let view = log_view(vec![log_item("(elided revisions)", None, None)]);
...
fn switch_mode_error_keeps_prior_view_state() {
    let mut view = log_view(vec![log_item("first", Some("first"), None)]);
...
fn reveal_change_id_switches_mode_when_current_mode_hides_change() {
    let mut view = log_view(vec![log_item("trunk", Some("trunk"), None)]);
...
fn toggle_select_requires_exact_change_id() {
    let mut view = log_view(vec![log_item("(elided revisions)", None, None)]);
...
fn toggle_select_tracks_exact_change_ids() {
    let mut view = log_view(vec![log_item("first", Some("change"), None)]);
    assert!(view.test_selected_change_ids().is_empty());
...
fn open_action_menu_prefers_single_row_context() {
    ...
    let menu = if let ViewEffect::OpenActionMenu(menu) = effect {
        menu
    } else {
...
fn open_action_menu_uses_bare_split_for_visible_working_copy() {
    ...
    assert!(matches!(
        ...
        crate::menus::FollowUp::SplitCurrentWorkingCopy
    ));
}
...
#[test]
fn open_action_menu_orders_new_parents_by_log_rows() {
    let mut view = log_view(vec![
        log_item("first", Some("aaaaaaaa"), None),
        log_item("second", Some("bbbbbbbb"), None),
    ...
    assert!(matches!(
        ...
        crate::menus::FollowUp::NewParents { parents }
...
fn current_row_highlight_preserves_rendered_foreground() {
    let view = log_view(vec![LogItem::new(
        vec![Line::styled(
            "colored row",
