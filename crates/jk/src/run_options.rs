...
use jk_tui::styles::{dialog, dialog_accent, dialog_error, dialog_supporting, dialog_warning};
use ratatui::Frame;
...
impl RunOptionsDialog {
    ...
    fn body(&self) -> Text<'static> {
        ...
        if let Some(error) = &self.error {
            lines.push(Line::styled(error.clone(), dialog_error()));
        }
        lines.push(Line::from(""));
        match self.command() {
            Ok(spec) => lines.push(Line::from(spec.command_preview().command_line)),
            Err(error) => lines.push(Line::styled(error, dialog_error())),
        }
        Text::from(lines)
    }

    /// Draws an opaque borderless panel and a scrollable, untruncated scope/command body.
    pub(crate) fn render(&mut self, frame: &mut Frame<'_>) {
        ...
        frame.render_widget(
            Paragraph::new(Line::from(vec![
                Span::styled("Run options", dialog_accent()),
                Span::styled(" · this command only", dialog_supporting()),
            ])),
    ...
    fn render_rows(&self, frame: &mut Frame<'_>, inner: Rect) {
        for (index, row) in ROWS.iter().copied().enumerate() {
            ...
            let line = Line::from(vec![
                ...
                Span::raw(format!("{:<17} {}", row.label(), self.row_value(row))),
            ]);
            frame.render_widget(
                Paragraph::new(line),
    ...
    fn render_footer(&self, frame: &mut Frame<'_>, inner: Rect) {
        ...
        if self.editing {
            ...
            frame.render_widget(
                Paragraph::new(format!("ID: {input}")).style(dialog_accent()),
            ...
            frame.render_widget(
                Paragraph::new(Line::from(vec![
                    ...
                    Span::raw(" done  "),
                    Span::styled("Ctrl-u", dialog_accent()),
                    Span::raw(" latest  "),
                    Span::styled("Esc", dialog_accent()),
                    Span::raw(" cancel"),
                ]))
                .style(dialog_supporting()),
            ...
            if self.max_scroll > 0 {
                controls.extend([
                    Span::raw("  "),
                    Span::styled("PgUp/PgDn", dialog_accent()),
                    Span::raw(" scroll"),
                ]);
            }
            frame.render_widget(
                Paragraph::new(Line::from(controls)).style(dialog_supporting()),
                Rect::new(inner.x, footer_y, inner.width, 1),
            );
            frame.render_widget(
                Paragraph::new(Line::from(vec![
                    Span::styled("a", dialog_accent()),
                    Span::raw(" apply to preview  "),
                    Span::styled("Esc", dialog_accent()),
                    Span::raw(" cancel"),
                ]))
                .style(dialog_supporting()),
...
mod tests {
    ...
    #[test]
    fn options_fill_the_panel_and_preserve_surrounding_repository_cells() {
        let mut options = RunOptionsDialog::new(rebase_spec()).expect("preview");
        ...
        let panel_rows = (0..40)
            .filter(|y| Some(buffer[(60, *y)].bg) == surface.bg)
            .collect::<Vec<_>>();
        let top = *panel_rows.first().expect("opaque panel");
        let height = panel_rows.len() as u16;
        assert!(top > 0 && top + height < 40);
        let panel = Rect::new(19, top, 82, height);
        for y in 0..40 {
            for x in 0..120 {
                let cell = &buffer[(x, y)];
        ...
        assert_eq!(Some(buffer[(20, top)].fg), dialog_accent().fg);
        assert_eq!(Some(buffer[(20, top + 2)].fg), dialog_accent().fg);
        assert_eq!(Some(buffer[(22, top + 2)].fg), surface.fg);
        assert_eq!(
            Some(buffer[(22, top + height - 1)].fg),
            dialog_supporting().fg
        );
        assert_eq!(Some(buffer[(20, top + height - 1)].fg), dialog_accent().fg);
    }
