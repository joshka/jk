...
pub fn confirm_command_preview_with_runner<R: JjCommandRunner>(
    ...
    match runner.run_confirmed_mutation(&pending.preview.spec) {
        Ok(output) if output.status.success() => refresh_after_mutation(state, source),
        Ok(output) => {
            let message =
...
fn refresh_after_mutation(state: &mut AppState, source: &mut JjLog) {
    let AppView::Log(log) = state.views.active_mut() else {
        return;
    };

    let refreshed = crate::refresh_log(
        log,
        &mut state.history,
        source,
