...
use crate::styles::{FOCUS, SUPPORTING, SURFACE, TITLE, WARNING};
...
impl CommandPreviewView {
    ...
    pub fn render(&self, frame: &mut Frame<'_>) {
        ...
        frame.render_widget(Block::default().style(SURFACE), panel);
        ...
        frame.render_widget(
            Paragraph::new(self.preview.title.as_str()).style(TITLE),
            header_area,
        );
        frame.render_widget(
            Paragraph::new(body)
...
fn footer_text(run_options: bool, scrolls: bool, width: u16) -> Text<'static> {
    ...
    if run_options {
        ...
        spans.extend([Span::styled("o", FOCUS), Span::raw(" options")]);
    }
    if scrolls {
        ...
        spans.push(Span::styled(hint, SUPPORTING));
    }
    lines.push(Line::from(spans));
    Text::from(lines)
}
...
mod tests {
    ...
    #[test]
    fn preview_inherits_terminal_colors_and_has_no_borders() {
        let preview = JjCommandSpec::confirm_mutation(
        ...
        assert!(
            buffer
                .content
                .iter()
                .all(|cell| { matches!(cell.fg, Color::Reset | Color::Yellow | Color::Cyan) })
        );
    }
    ...
    fn narrow_preview_keeps_the_complete_command_inspectable() {
        ...
        let view = CommandPreviewView::new(preview);
        let area = Rect::new(0, 0, MIN_CONFIRM_WIDTH, MIN_CONFIRM_HEIGHT);
    ...
    #[test]
    fn overlay_clears_underlying_text_with_terminal_default_surface() {
