...
impl TryFrom<BookmarkRecord> for BookmarkRef {
    ...
    fn try_from(record: BookmarkRecord) -> Result<Self, Self::Error> {
        ...
        // A tracked remote can point at a locally deleted bookmark. In that case jj emits an empty
        // tracking target; the field's presence, not its length, carries tracking state.
